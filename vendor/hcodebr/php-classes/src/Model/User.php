...
class User extends Model
{
    ...
    {
        $sql = new Sql();
        $sql = $sql->query("CALL sp_users_delete(:iduser)", [
            ":iduser" => $this->getiduser()
        ]);
    }
    ...
    public static function getForgot($email, $inadmin = true)
    {
        ...
        if(count($results) === 0) {
            ...
            if(count($results2) === 0) {
                ...
                if ($inadmin === true) {
                    $link = "http://localhost:8000/admin/forgot/reset?code=$result";
                } else {
                    $link = "http://localhost:8000/forgot/reset?code=$result";
                } 
    ...
    public static function validForgotDecrypt($result)
    {
        ...
        $results = $sql->select(
            ...
            WHERE 
             a.idrecovery = :idrecovery AND a.dtrecovery IS NULL AND DATE_ADD(a.dtregister, INTERVAL 1 HOUR) >= NOW()"
        ,[
